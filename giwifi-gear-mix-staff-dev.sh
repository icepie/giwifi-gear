...
gw_logout() {
	...
	#:8060/wifidog/userlogout?ip=&mac=
}
...
gw_get_login_page() {
	echo $(curl -s -L -A "$AUTH_UA" "$(gw_get_auth_url $1)&account_type=1" | grep "name=")
}
...
gw_loginaction() {
	...
	echo $(json_format "$(
		...
		-d "$1" \
		"http://login.gwifi.com.cn/cmps/admin.php/Api/loginaction" | gunzip
	)")
...
&user_agent=$(get_json_value $GW_AUTH_STATE_RTE_DATA 'user_agent')\\
&link_data=$(get_json_value $GW_AUTH_STATE_RTE_DATA 'link_data')\
""" | sed 's/"//g')
